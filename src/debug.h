...
// enhanced assert? check that A is true, otherwise log error M then jump to
// error: for cleanup
#define check(A, M, ...)                                                       \
  if (!(A)) {                                                                  \
...
// placed in part of fn that shouldn't run, i.e. if/switch branches, prints
// error followed by jumping to error: cleanup
#define sentinel(M, ...)                                                       \
  {                                                                            \
    log_err(M, ##__VA_ARGS__);                                                 \
    errno = 0;                                                                 \
...
// same as check, except that error only gets reported when ndebug flag is not
// set
